	...
	>
        ...
        >
            {#each images as image}
                <a class="shrink-0 grow flex w-full h-max md:w-max md:h-full lg:w-full lg:h-max snap-start" href="#{image}">
                    <!-- svelte-ignore a11y-missing-attribute -->
                    <img
                        src="/processed_photos/{image}.jpg"
                        class=" 
                w-full h-full {($page.url.hash.split('#')[1] || '0')==image.toString()?"md:brightness-75":"md:brightness-50"}"
                    />
                </a>
            {/each}
            <div class="w-full snap-start"></div>
        </div>
		<div
			class="
        hidden md:flex
        ...
        <div class="grow flex flex-col lg:pr-7 lg:order-2 collapse md:visible">
            ...
            flex flex-col
            primary"
            style='background-image: url("/processed_photos/{$page.url.hash.split('#')[1] || '0'}.jpg");'
            >
